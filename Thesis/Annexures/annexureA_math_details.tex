...
% -------------------------------------------------------------
\subsection{Derivation of the \gls{adam} and \gls{nadam} Update Rules}
\label{annexure:adam_nadam}

\noindent\textbf{\Gls{adam} update rule.}
...
\noindent\textbf{\Gls{nadam} update rule.}
\Gls{nadam} incorporates Nesterov momentum by replacing the bias-corrected first moment $\hat{m}_t$ with a look-ahead estimate. The modification replaces~\eqref{eq:adam_update} with
\begin{equation}
...
\noindent\textbf{Axiom 1: Efficiency.}
\[
  \sum_{j=1}^{p} \phi_j(v) = v(N) - v(\emptyset).
\]

\noindent\textbf{Axiom 2: Symmetry.}
...
There exists a unique function $\phi : \mathcal{G}^N \to \mathbb{R}^p$ satisfying Axioms 1 to 4, and it is given by
\[
