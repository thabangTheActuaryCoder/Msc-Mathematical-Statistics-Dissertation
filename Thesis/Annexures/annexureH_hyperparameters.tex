...
\begin{enumerate}
    \item Activation function (\gls{celu}, \gls{gelu}, Tanh), with fixed lr${}=0.001$, batch size${}=256$, \gls{adam}.
    \item Learning rate ($0.01, 0.001, 0.0005, 0.0001$), with fixed best activation, batch size${}=256$, \gls{adam}.
    \item Batch size ($64, 128, 256, 512$), with fixed best activation and learning rate, \gls{adam}.
    \item Optimiser (\gls{adam} vs \gls{nadam}), with all previous best values fixed.
    \item Momentum $\beta_1$ ($0.90, 0.95, 0.99$) with $\beta_2 = 0.999$ fixed, using the best optimiser.
\end{enumerate}

Each configuration was trained with early stopping (patience of 10 validation checks at 5-epoch intervals) up to a maximum of 200 epochs. Table~\ref{tab:hyper_full} reports the full results.
...
\begin{tabular}{llrrr}
\toprule
\textbf{Phase} & \textbf{Configuration} & \textbf{Best Epoch} & \textbf{Val \gls{r2}} & \textbf{Time (s)} \\
\midrule
\multirow{3}{*}{Phase 1: Activation}
  & \gls{celu}  & 100 & 0.995682 & 556.7 \\
  & \gls{gelu}  &  25 & 0.995636 & 313.8 \\
...
\multirow{2}{*}{Phase 4: Optimiser}
  & \gls{adam}  & 100 & 0.995692 & 562.7 \\
  & \gls{nadam} &  85 & 0.995693 & 582.5 \\
\midrule
...
The following figures present the validation \gls{mse} loss and validation \gls{r2}
...
\caption{Phase~1: validation \gls{mse} loss (left) and validation \gls{r2}
...
\caption{Phase~2: validation \gls{mse} loss (left) and validation \gls{r2}
...
\caption{Phase~3: validation \gls{mse} loss (left) and validation \gls{r2}
(right) by batch size.}
...
\caption{Phase~4: validation \gls{mse} loss (left) and validation \gls{r2}
(right) by optimiser.}
...
\caption{Phase~5: validation \gls{mse} loss (left) and validation \gls{r2}
...
Loss function   & \gls{mse} (on normalised scale) \\
\bottomrule
\end{tabular}
\caption{Best hyperparameter configuration selected for the final \gls{dnn} model.}
