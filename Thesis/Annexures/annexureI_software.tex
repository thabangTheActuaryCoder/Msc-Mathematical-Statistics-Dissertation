...
scikit-learn      & 1.3+             & Random Forest, GP surrogate, train/test splitting, metrics \\
...
The PyTorch device was selected automatically: MPS (Metal Performance Shaders) if available on Apple Silicon, CUDA if available on NVIDIA hardware, and CPU as fallback.
...
\begin{itemize}
    ...
    \item Random Forest fitting (\texttt{random\_state=42})
...
\begin{itemize}
    ...
    \item Parametric Monte Carlo simulation (Annexure~\ref{annexure:mc_simulation}).
    \item GLM benchmark fitting and comparison (Annexure~\ref{annexure:glm_benchmark}).
    \item Tree-based benchmark fitting: Random Forest and XGBoost (Section~\ref{sec:tree_benchmarks_methods}).
