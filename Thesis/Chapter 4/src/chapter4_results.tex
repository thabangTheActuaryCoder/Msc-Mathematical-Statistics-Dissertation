...
\gls{dnn}.  Both models were fitted on the same training partition
($n = 524\,288$) using the same encoded feature set and the same
train/validation/test split, ensuring that the comparison reflects
genuine model-class differences rather than data-handling artefacts.
...
= 141.0 / 44.3 = 3.18$ matches the theoretical ratio
$\sqrt{10\,000 / 1\,000} = \sqrt{10} \approx 3.16$, and
$\mathrm{MCSE}(B{=}1\,000) \,/\, \mathrm{MCSE}(B{=}20\,000)
= 141.0 / 31.4 = 4.49$ matches $\sqrt{20} \approx 4.47$.  The
...
quantile estimation precision.  The 90th percentile varies within a band
of R182 across simulation sizes (R22\,490 to R22\,672), the 95th
percentile within R298 (R24\,030 to R24\,328), and the 99th percentile
within R532 (R27\,131 to R27\,663).  The progressively wider variation
at higher quantiles reflects the decreasing effective sample size in the
tail: at $\tau_{99}$ with $B = 10\,000$, only $100$ observations
determine the percentile estimate, compared with $1\,000$ at
$\tau_{90}$ and $500$ at $\tau_{95}$.  Nonetheless, even at
$\tau_{99}$, the running percentile has stabilised to within
$\pm 1.0\%$ of its final value from $B = 5\,000$ onwards, indicating
that the tail percentile estimates are sufficiently precise for the
conditional Shapley analysis that follows.
...
$|\mathcal{H}_{0.99}| = 100$ profiles, which warrants scrutiny of its
statistical reliability.  Three lines of evidence support the robustness
of this finding.  First, the reversal is not an isolated artefact at a
single threshold: the conditional importance values for
\texttt{coverage\_level}, \texttt{medical\_history} and
...
\texttt{family\_medical\_history} exceeds \texttt{smoker} by $0.056$ on
the normalised scale ($0.622$ versus $0.566$), a relative difference of
$9.9\%$.  Third, the profile-level composition analysis
...
these three independent lines of evidence supports the conclusion that
