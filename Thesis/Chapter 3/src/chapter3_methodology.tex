...
$60/20/20$ partition.  The role of each partition is as described in
...
the validation MSE is equivalent to maximising the validation $R^2$; the
dot-product $R^2$ is therefore used as the operational model-selection
...
Training stops early if the validation $R^2$ does not improve for $P = 10$
consecutive validation checks \citep{prechelt2012early}; the retained
model is the checkpoint with the highest validation $R^2$ observed.  The test set is reserved
...
Secondary evaluation metrics & MAE, RMSE and $R^2$ on the original response scale \\
...
Model-selection criterion & Dot-product validation $R^2$ \\
Early stopping & Patience $P = 10$ validation checks without $R^2$ improvement \\
...
(3)~validation evaluation every 5 epochs using the dot-product $R^2$;
(4)~early stopping when the validation $R^2$ has not improved for 10
consecutive checks; (5)~selection of the checkpoint with the highest
validation $R^2$; and (6)~final test-set evaluation on the original
monetary scale using $R^2$, RMSE and MAE.  The complete
...
\caption{\Gls{pca} projection of joint input structure: training, MC sample and overlay.}
...
The prediction function supplied to \gls{shap} is the fitted DNN
