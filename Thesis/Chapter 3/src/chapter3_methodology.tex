...
\noindent
This chapter translates the conceptual DNN--Monte Carlo--Shapley framework
...
\textit{Insurance Data for Machine Learning}
...
\texttt{age} & Predictor, numerical & Age (years), $18$--$65$ & One numerical input, standardised \\
\texttt{bmi} & Predictor, numerical & Body mass index, $18.00$--$50.00$ & One numerical input, standardised \\
\texttt{children} & Predictor, count & Number of dependants, $0$--$5$ & One numerical input, standardised \\
...
pipeline is audited in Annexure~G.
...
in Chapter~\ref{sec:loss_optimisation}.  Validation performance is
...
The base simulation uses $B = 10\,000$ generated policyholder profiles.  A
pilot convergence check is conducted by repeating the simulation for
increasing values of $B \in \{1\,000,\ 2\,500,\ 5\,000,\ 10\,000,\
20\,000\}$.  For each $B$, the running mean, standard deviation, and
...
Chapter~\ref{sec:explainability_shapley_shap}.  As discussed there,
...
\textit{approximate} Shapley values, not exact game-theoretic Shapley
...
Chapter~\ref{sec:explainability_shapley_shap}.  In the present context, the
...
three-stage pipeline are presented in Chapter~\ref{sec:results}.
