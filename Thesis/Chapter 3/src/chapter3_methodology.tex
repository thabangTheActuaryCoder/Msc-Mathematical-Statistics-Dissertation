...
using Kaiming uniform initialisation \citep{he2015delving};
(2)~mini-batch gradient descent with the NAdam optimiser;
...
(Annexure~\ref{annexure:dnn_algorithm_detailed}).
...
The GLM is fitted via iteratively reweighted least squares (IRLS),
...
\widehat{\boldsymbol{\beta}})$.  The complete IRLS algorithm is
presented in Algorithm~\ref{tab:glm_algorithm_annex}
(Annexure~\ref{annexure:glm_algorithm}).
...
The training procedure grows $B = 500$ unpruned trees on bootstrap
samples, with $m = \lfloor p/3 \rfloor$ candidate predictors evaluated
at each split.  The split criterion minimises the weighted residual sum
of squares (equation~\eqref{eq:rf_split_criterion}), and splitting
continues until each terminal node contains at most $n_{\min} = 5$
observations.  The ensemble prediction is the average across all trees.
The complete algorithm is presented in
Algorithm~\ref{tab:rf_algorithm_annex}
(Annexure~\ref{annexure:rf_algorithm}).
...
The training procedure begins with a constant initialisation and
iteratively adds depth-limited regression trees, each fitted to the
gradient and Hessian statistics of the current residuals
(equation~\eqref{eq:xgb_gradient_hessian}).  Splits are selected to
maximise the gain (equation~\eqref{eq:xgb_split_gain}), and the
optimal leaf weights are computed in closed form
(equation~\eqref{eq:xgb_optimal_leaf}).  Validation-based early
stopping with patience $P = 50$ rounds terminates training when the
validation loss ceases to improve.  The complete algorithm is presented
in Algorithm~\ref{tab:xgb_algorithm_annex}
(Annexure~\ref{annexure:xgb_algorithm}).
