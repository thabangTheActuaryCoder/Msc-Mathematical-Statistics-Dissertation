...
\noindent
This chapter translates the conceptual DNN, MC and Shapley framework
...
and training protocol of the DNN that serves as the prediction
function, together with the GLM and tree-based benchmark models,
...
Section~\ref{sec:mc_simulation_design} sets out the parametric MC
...
column, \texttt{charges}, is the continuous monetary outcome.  The terms
\emph{cost} and \emph{charge} are used interchangeably throughout the
dissertation to denote this same modelled quantity, and a
\emph{predicted cost} is always the fitted value of \texttt{charges}
returned by a model.  \emph{Premium}, the price actually quoted to a
policyholder, is a distinct quantity that incorporates expense and profit
loadings and is not modelled here; where the word appears it refers to the
literature being reviewed rather than to the response of this study.
...
\newcommand{\nohyph}{\raggedright\arraybackslash
  \hyphenpenalty=10000 \exhyphenpenalty=10000}
\begin{tabular}{%
  >{\nohyph}p{0.22\textwidth}%
  >{\nohyph}p{0.17\textwidth}%
  >{\nohyph}p{0.31\textwidth}%
  >{\nohyph}p{0.20\textwidth}}
\toprule
...
\texttt{bmi} & Predictor, numerical & BMI, $18.00$ to $50.00$ & One numerical input, standardised \\
...
MC simulated profiles.  We denote this complete preprocessing
procedure by $T(\cdot)$.  The treatment of categorical variables during MC simulation,
...
\begin{enumerate}
    ...
    \item \textbf{Training-only standardisation.}  The validation and test
          ...
          and~\eqref{eq:response_standardisation_ch3}).  MC
...
uniform marginal distributions in the MC simulation design.  The
...
empirical scale references; in the later MC and Shapley stages,
...
product-marginal independence assumption used in the MC
...
the fitted category proportions used in the MC simulation are
...
\begin{enumerate}
    ...
    \item The weak linear correlations among the numerical predictors,
          ...
          product-marginal independence assumption adopted in the MC
...
The predictive component is a DNN for supervised
...
\begin{figure}[htbp]                                                                                                                                                                                               
  ...
  \end{tikzpicture}
  \caption{Funnel architecture of the DNN.}
...
(equation~\eqref{eq:celu}).  CELU was selected from among the
...
problem associated with ReLU while maintaining a linear positive
...
restriction rather than a costless simplification: because the CELU
...
The network is trained by minimising the MSE loss
...
derivative of the activation function.  For the CELU activation
...
The network is trained by minimising the MSE on the
...
the validation MSE is equivalent to maximising the validation \ensuremath{R^2}; the
dot-product \ensuremath{R^2} is therefore used as the operational model-selection
...
Training stops early if the validation \ensuremath{R^2} does not improve for $P = 10$
consecutive validation checks \citep{prechelt2012early}; the retained
model is the checkpoint with the highest validation \ensuremath{R^2} observed.  The test set is reserved
...
(equation~\eqref{eq:r_squared}) is reported alongside the MAE and
RMSE.
...
Architecture type & Fully connected DNN \\
...
Training objective & MSE on the standardised response scale \\
Secondary evaluation metrics & MAE, RMSE and \ensuremath{R^2} on the original response scale \\
...
Model-selection criterion & Dot-product validation \ensuremath{R^2} \\
Early stopping & Patience $P = 10$ validation checks without \ensuremath{R^2} improvement \\
...
\bottomrule
\end{tabular}
\caption{DNN architecture and training configuration.}
...
(3)~validation evaluation every 5 epochs using the dot-product \ensuremath{R^2};
(4)~early stopping when the validation \ensuremath{R^2} has not improved for 10
consecutive checks; (5)~selection of the checkpoint with the highest
validation \ensuremath{R^2}; and (6)~final test-set evaluation on the original
monetary scale using \ensuremath{R^2}, RMSE and MAE.  The complete
...
A classical actuarial benchmark is provided by a GLM
...
predictors used by the DNN), the GLM assumes
\begin{equation}
...
log-likelihood for the gamma GLM is
\begin{equation}
...
The parameter vector $\boldsymbol{\beta}$ is estimated by IRLS, which solves the score equations
\begin{equation}
    \sum_{i=1}^{n}
...
Two tree-based ensemble methods are included alongside the GLM to
...
An RF regressor \citep{breiman2001random} is fitted with 500 trees,
maximum depth unrestricted, minimum samples per leaf of 5, and
\texttt{random\_state=42}.  RF aggregates predictions across
decorrelated decision trees, reducing variance through bootstrap
aggregation (bagging) while maintaining low bias.

The RF constructs an ensemble of $B$ regression trees, each grown on
...
$\hat{y}_b(\mathbf{z}) = T_b(\mathbf{z})$.  The RF prediction is
...
\end{equation}
\equationlistentry{RF ensemble prediction}
...
\end{equation}
\equationlistentry{RF split criterion}
...
Trees are grown until each terminal node contains at least 5 observations.

The variance reduction mechanism of the RF can be understood
...
\end{equation}
\equationlistentry{RF variance reduction}
...
predictor subset, the RF reduces $\rho$ below the value obtained by
...
Table~\ref{tab:rf_hyperparameters_ch3} summarises the RF
configuration.
...
Split criterion & MSE (equation~\eqref{eq:rf_split_criterion}) \\
...
A gradient-boosted tree regressor is fitted using the XGBoost
...
validation set with a patience of 50 rounds.  XGBoost constructs an
...
scales each tree's contribution.  The shrinkage applies only to the
trees fitted at rounds $m \ge 1$; the initial constant enters
unshrunk, so that the ensemble begins at the training-set mean and each
subsequent tree adds a damped correction to it.

At each boosting round $m$, XGBoost fits a new tree $h_m$ to
...
The key computational innovation of XGBoost is the use of a
...
left ($I_L$) and right ($I_R$) child nodes, XGBoost computes the
gain:
\begin{equation}
    \mathrm{Gain}
    =
...
Table~\ref{tab:xgb_hyperparameters_ch3} summarises the XGBoost
configuration.
...
cross-validation, paralleling the no-tuning treatment of the GLM;
...
models are evaluated using $R^2$, RMSE and MAE on the
...
purpose: to verify that the fitted DNN is sufficiently stable and
well-behaved to act as the prediction surface in the subsequent MC
...
\begin{enumerate}
    ...
    \item residual distribution comparisons across the training, validation
          and test partitions.
\end{enumerate}
The DNN does not require normally distributed residuals; these checks
are purely descriptive.  The fitted residuals are also retained for the
residual-augmented MC sensitivity analysis described later.
...
not itself a concern for the DNN, which does not assume normality of
...
PCA is employed as a structural diagnostic of the standardised,
...
DNN has captured the dominant target structure visible in the input
space.
...
\end{enumerate}
Comparing these views indicates whether the fitted DNN has preserved
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Thesis/Chapter 3/Figures/fig_sml_heatmap_train_actual.png}
\caption{Input-target correlation heatmap: training set, actual charges.}
\label{fig:heatmap_train_actual_ch3}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Thesis/Chapter 3/Figures/fig_sml_heatmap_train_predicted.png}
\caption{Input-target correlation heatmap: training set, predicted charges.}
\label{fig:heatmap_train_predicted_ch3}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Thesis/Chapter 3/Figures/fig_sml_heatmap_test_actual.png}
\caption{Input-target correlation heatmap: test set, actual charges.}
\label{fig:heatmap_test_actual_ch3}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Thesis/Chapter 3/Figures/fig_sml_heatmap_test_predicted.png}
\caption{Input-target correlation heatmap: test set, predicted charges.}
\label{fig:heatmap_test_predicted_ch3}
\end{figure}

\noindent
Figures~\ref{fig:heatmap_train_actual_ch3}
to~\ref{fig:heatmap_test_predicted_ch3} present the four correlation
...
surface for the MC and Shapley-value analyses that follow.
...
The MC stage generates a synthetic policyholder input space and
...
DNN computes predicted costs for the generated profiles.  The
...
\end{equation}
\equationlistentry{Product-marginal MC input distribution}
...
\bottomrule
\end{tabular}
\caption{MC input distributions for raw policyholder variables.}
...
MCSE formula does not apply to quantiles: for a quantile level
...
MCSE & Quantifies simulation precision for selected summaries \\
\bottomrule
\end{tabular}
\caption{MC convergence quantities recorded during the pilot run.}
...
the model-ready input vector $\mathbf{z}^{(b)}$.  The fitted DNN then
calculates the base MC prediction, following the formulation in
...
\end{equation}
\equationlistentry{Base MC DNN prediction}
...
\toprule
\textbf{Algorithm 3.1: Base parametric MC simulation} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Base MC simulation algorithm.}
...
MC sample. The element-wise difference
\begin{equation}
    \Delta_{jk}
    =
    r_{jk}^{(\mathrm{train})}
...
MC sample draws each raw variable independently, the correlations
...
\centering
\includegraphics[width=0.90\textheight]{Thesis/Chapter 3/Figures/fig_mc_joint_correlation_comparison.png}
...
partition and on the parametric MC sample. All pairwise values of
...
The third diagnostic is a PCA overlay of the joint input space.
...
\begin{sidewaysfigure}
\centering
\includegraphics[width=0.90\textheight]{Thesis/Chapter 3/Figures/fig_mc_joint_pca_comparison.png}
...
\noindent
The MC sample populates the same region of principal component
...
marginal distribution choice, a second MC sample of size $B$ is
...
\end{equation}
\equationlistentry{Bootstrap MC resampling from empirical distribution}
...
and scored through the fitted DNN in the same manner as the
...
\bottomrule
\end{tabular}
\caption{Comparison of the two MC input regimes.}
...
DNN's internal importance structure rather than by the particular
...
Shapley analysis. The fitted DNN's internal importance structure,
...
A second MC design assesses sensitivity to fitted residual
variation. The base MC design produces deterministic fitted
...
distributional conclusions drawn from the base MC analysis are
...
For each base MC prediction $\widehat{Y}^{(b)}$, a residual draw is
sampled:
\begin{equation}
    \varepsilon^{(b)}
    \sim
...
\bottomrule
\end{tabular}
\caption{Residual-augmented MC sensitivity design.}
...
\begin{tabular}{p{0.92\textwidth}}
\toprule
\textbf{Algorithm 3.2: Residual-augmented MC sensitivity simulation} \\
\midrule
\textbf{Input:} base MC predictions
...
8. Store all outputs for comparison with the base MC distribution. \\
\bottomrule
\end{tabular}
\caption{Residual-augmented MC algorithm.}
...
Shapley-value methods are used to explain the fitted DNN predictions
calculated for simulated policyholder profiles. The attribution analysis is
connected to the MC predicted-cost distribution, with particular
attention to the upper tail.

\subsubsection{Prediction function and Shapley decomposition}

The prediction function supplied to SHAP is the fitted DNN
...
differentiable DNN. A model-agnostic Kernel SHAP calculation may be
...
of the fitted DNN to estimate feature contributions efficiently, but
...
Explained profiles & MC simulated profiles $\mathbf{z}^{(b)}$ \\
...
The fitted DNN receives $p = 22$ encoded features, but some columns are
...
base MC simulation, so $B_{\mathrm{shap}} = B$. Global feature
...
The joint MC input law is a practical simulation assumption and not a proof of covariate independence; validated by joint-distribution diagnostics and Shapley invariance comparison (Section~\ref{sec:mc_input_validation}) \\
