...
outlined a three-stage framework that combines \gls{dnn} regression,
\gls{mc} simulation and Shapley-value attribution.  This chapter develops
...
\noindent
The review begins with the integrated \gls{dnn}, \gls{mc} and Shapley framework
...
predictive formulation; \gls{mc} methods extend the fitted prediction
...
% =============================================================
% 2.2  DNN-\gls{mc}-Shapley Framework
% =============================================================

\subsection{\Gls{dnn}-\gls{mc}-Shapley Framework}
\label{sec:integrating_dnn_mc_shapley}

\noindent
\gls{mc} simulation is a long-established framework for analysing the
...
for nonlinear fitted models such as \glspl{dnn}
...
\citep{lundberg2017unified, rozemberczki2022shapley}. \gls{mc} simulation
...
the network has been fitted, \gls{mc} simulation is used to generate a
...
justified: the \gls{dnn} provides the fitted prediction surface, \gls{mc}
...
modelling tradition. The review supports the choice of a \gls{dnn}
...
larger synthetic policyholder population generated by \gls{mc} simulation,
...
\gls{mc} simulation over a synthetic policyholder population and Shapley
...
\end{equation}
\equationlistentry{Fitted \gls{dnn} prediction function}
...
manually, as would be necessary in a \gls{glm} or polynomial
...
\glspl{glm}, tree-based models or other actuarial models for
...
% --------------------------------------------------------
% 2.2.2 \gls{mc} Simulation
% --------------------------------------------------------
\subsubsection{\gls{mc} Simulation}
\label{montecarlo_propagation}

\noindent
The \gls{mc} stage studies how the fitted prediction surface
...
\label{eq:mc_sample_framework}
    ...
    \widehat{F}_{X},
\end{equation}
\equationlistentry{\gls{mc} sample from input distribution}
...
\label{eq:mc_predictions_framework}
    ...
    \qquad \ell = 1, \ldots, M.
\end{equation}
\equationlistentry{Fitted predictions for \gls{mc} samples}
...
training. Residual-augmented \gls{mc} and predictive-uncertainty analyses
...
\gls{mc} input simulation must be distinguished from \gls{mc}
...
This notation separates the input distribution used for \gls{mc} simulation,
...
profile.
  ...
  \noindent
  When exact Shapley values are not computed, a permutation-based \gls{mc}
...
\label{sec:statistical_learning_supervised_regression}
  ...
  \noindent
  The predictive component of the \gls{dnn}-\gls{mc}-Shapley workflow is a
...
Regression methods are evaluated on a continuous scale using prediction-error measures \citep{james2021introduction,hastie2009elements}. The specific measures employed in this dissertation are defined below.
  ...
  \label{eq:mse}
      ...
      \right)^{2}.
  \end{equation}
  \equationlistentry{\Gls{mse}}
  The \gls{rmse} is the square root of the \gls{mse},
  \begin{equation}
  \label{eq:rmse}
      \mathrm{RMSE}
      =
  ...
  of the response variable. The \gls{mae} is
  \begin{equation}
  \label{eq:mae}
      \mathrm{MAE}
      =
  ...
  \Glspl{glm} extend this formulation by specifying a distributional
  ...
  \label{eq:glm_link}
      ...
      \beta_{j}\,x_{ij}.
  \end{equation}
  \equationlistentry{\Gls{glm} link function}
  ...
  \gls{glm} modelling has a long history in insurance pricing and risk modelling \citep{dobson2008introduction,denuit2007actuarial}. A purely linear predictor, however, cannot represent nonlinear predictor effects without explicitly specifying transformations or basis expansions in advance.

  \noindent
  Because \glspl{glm} remain the industry-standard framework for health
  insurance pricing, a gamma \gls{glm} with a logarithmic link function serves as a
...
\(\widehat{m}\) that can be evaluated repeatedly in the \gls{mc} and
...
function that can be repeatedly evaluated and then connected to \gls{mc}
...
function \(\widehat{m}(\cdot)\). \gls{mc} simulation subsequently studies the
...
\newpage
% =============================================================
% 2.4  \Glspl{dnn} for Regression
% =============================================================

\subsection{\Glspl{dnn} for Regression}
\label{sec:dnn_regression}

\noindent
A \gls{dnn} is a composition of layers that maps an input vector to an
...
conditions under which \glspl{dnn} are competitive with tree-based
...
\label{eq:dnn_prediction}
    ...
    a^{(L)}_{i1},
\end{equation}
\equationlistentry{Fitted \gls{dnn} prediction for observation $i$}
...
tangent function, the \gls{relu}, the \gls{celu} and the \gls{gelu}
...
prediction, \gls{mse} is a natural objective because it penalises large
...
are well suited to gradient-based optimisation.  The \gls{mse} loss is
defined by
\begin{equation}
\label{eq:mse_loss}
    \mathcal{L}(\theta)
    ...
    \right)^2,
\end{equation}
\equationlistentry{\Gls{mse} loss}
...
\noindent
\Gls{nadam} modifies Adam by incorporating a
...
is important because the subsequent \gls{mc} and Shapley analyses are applied
...
\gls{dnn} prediction, \gls{mc} input simulation and Shapley-value
...
regression-based models, especially \glspl{glm}, while the
...
established regression methods such as \glspl{glm}, as measured
...
that the network's \Gls{ice} outputs vary in a manner
...
\citep{orji2024explainable} apply \glspl{rf} and gradient-boosted trees to
...
\citep{friedman2001greedy}, frequently match or outperform \glspl{dnn}
on
...
\gls{mc} and Shapley workflow because its smooth, differentiable prediction
...
the \gls{dnn}, \gls{mc} input simulation is used to generate many
...
\noindent
The interpretation remains model-based. The \gls{mc} distribution describes
...
training, validation and test structure, but \gls{mc} input simulation requires an
...
\(\widehat{F}_{X}\) used later in the \gls{mc} component of the framework.
...
even though \gls{mc} input simulation focuses primarily on the predictors.
...
profiles generated for \gls{mc} simulation remain compatible with that same
...
% =============================================================
% 2.8  Foundations of \gls{mc}
% =============================================================

\subsection{Foundations of \gls{mc}}
\label{sec:monte_carlo_foundations}

\noindent
\gls{mc} simulation rests on a small number of foundational results
concerning the crude \gls{mc} estimator, its convergence
...
then the crude \gls{mc} estimator of \(\theta\) is
\begin{equation}
\label{eq:mc_estimator}
    ...
    \right).
\end{equation}
\equationlistentry{Crude \gls{mc} estimator}
...
\end{equation}
\equationlistentry{Law of large numbers for \gls{mc}}
If
\begin{equation}
...
\label{eq:mc_estimator_variance}
    ...
    \frac{\sigma_g^2}{M},
\end{equation}
\equationlistentry{Variance of \gls{mc} estimator}
which yields the familiar \(M^{-1/2}\) convergence rate of ordinary \gls{mc}
simulation \citep{caflisch1998monte,glasserman2004monte}.

\noindent
The \gls{mcse} quantifies the simulation error associated with
...
\label{eq:mc_clt}
    ...
    N(0,1),
\end{equation}
\equationlistentry{Central limit theorem for \gls{mc}}
giving the approximate \(100(1-\alpha)\%\) \gls{mc} confidence interval
\begin{equation}
\label{eq:mc_ci}
    \widehat{\theta}_{M}
    \pm
    ...
    \frac{s_M}{\sqrt{M}}.
\end{equation}
\equationlistentry{\gls{mc} confidence interval}
This interval describes uncertainty due to the finite number of \gls{mc}
...
\noindent
For many quantities, ordinary \gls{mc} simulation is sufficient when the
simulation budget is large and the \gls{mcse} is reported.
However, when the quantity of interest is a tail probability, a high quantile or
a rare high-cost event, crude \gls{mc} can become inefficient because few
...
\(\widehat{Y}^{(1)},\ldots,\widehat{Y}^{(M)}\), the crude \gls{mc} estimator
of \(p_u\) is
\[
    \widehat{p}_u
    =
...
estimation can be difficult under crude \gls{mc} sampling.
...
\noindent
Quasi-\gls{mc} methods replace independent random points with
...
quasi-\gls{mc} methods can improve convergence relative to ordinary Monte
...
\[
    ...
    \right)
\]
\gls{mc} rate \citep{caflisch1998monte,lemieux2009monte,owen2013monte}.
...
crude \gls{mc} or empirical resampling, but the variance-reduction literature
...
precision of the reported \gls{mc} summaries.
...
\noindent
\Glspl{dnn} are flexible nonlinear prediction models, but their flexibility often comes at the cost of interpretability. In insurance applications, this is a serious practical and regulatory concern. A model may produce accurate predictions, but insurers, regulators and policyholders may still require an explanation of why a particular profile receives a high predicted cost. This motivates the use of explainable artificial intelligence methods \citep{lundberg2017unified, molnar2022interpretable}.
...
\noindent
The dissertation extends ordinary Shapley analysis by connecting it to \gls{mc} simulation. Instead of computing Shapley values only on the observed test set, the dissertation also examines Shapley values for simulated policyholder profiles, concentrating on the high-cost region of the model-implied prediction distribution. Let \(\widehat{Y} = \widehat{m}(\mathbf{X})\) denote the predicted cost for a simulated input profile and let \(\phi_j(\mathbf{X})\) denote the Shapley contribution of feature \(j\). For a high-cost threshold \(u\), the conditional Shapley contribution of feature \(j\) is
\[
\mathbb{E}
\!\left[
\phi_j(\mathbf{X})
...
the \gls{dnn} supplies a fitted nonlinear prediction surface, \gls{mc}
...
\noindent
\gls{mc} simulation has a long history in actuarial science and quantitative
...
use of \gls{mc} simulation to generate a synthetic input space and propagate
...
\noindent
At the prediction stage, several studies fit \glspl{dnn} or comparable
...
protocol, network architecture, \gls{mc} simulation design and
