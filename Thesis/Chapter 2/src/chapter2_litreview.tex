...
outcome \citep{hastie2009elements, james2021introduction}.  A \gls{dnn}
...
samiuddin2023health}.  The use of a \gls{dnn} in this role is motivated by
the universal approximation property: feedforward neural networks with
...
input configurations is examined through \gls{mc} simulation.  \Gls{mc}
...
closed-form analytical arguments.  In the framework adopted here, \gls{mc}
...
nonlinear model such as a \gls{dnn}, the relationship between inputs and
...
axioms\footnote{The four axioms are efficiency, symmetry, dummy and
additivity \citep{shapley1953value, lundberg2017unified}.} regardless of
the internal structure of the fitted function
...
\noindent
The three components are hierarchically justified: the \gls{dnn} provides
the fitted prediction surface, \gls{mc} simulation examines its behaviour
empirically where closed-form analysis is unavailable, and Shapley values
attribute predictions to input features where the network's parameters
cannot be interpreted directly.  Used jointly, the simulation and
attribution layers allow both the output distribution and the feature-level
drivers of selected regions of that distribution to be examined.  The
