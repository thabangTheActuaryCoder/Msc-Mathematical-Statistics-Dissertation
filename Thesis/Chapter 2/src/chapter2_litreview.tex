...
Measures such as RMSE, MAE and $R^2$ are informative about
...
machine-learning regressors together with SHAP and Individual Conditional Expectation (ICE) plots to
...
is \citet{samiuddin2023health}, who fit Artificial Neural Network (ANN) and DNN models to the
...
This notation separates the input distribution used for MC simulation,
...
Regression methods are evaluated on a continuous scale using prediction-error measures \citep{james2021introduction,hastie2009elements}. The specific measures employed in this dissertation are defined below.
  ...
  MAE and $R^2$ are reported together rather than treated as
...
tangent function, the Rectified Linear Unit (ReLU), the Continuously Differential Exponential Linear Unit (CELU) and the Gaussian Error Linear Unit (GELU)
...
\noindent
Nesterov-accelerated Adpative Moment Estimation (NAdam)
modifies Adam by incorporating a
...
is not adopted as the primary validation strategy. It would require
refitting the neural network once for each observation. This is computationally
inefficient for a DNN, since each fit involves iterative nonconvex
optimisation rather than a closed-form estimation step. In addition, the
training sets used in LOOCV differ by only one observation, so the
...
performance using RMSE, MAE and $R^2$. If
...
\citet{orji2024explainable} apply RF and gradient-boosted trees to
...
Chapter~\ref{chap:methods_and_data}.  A RF
