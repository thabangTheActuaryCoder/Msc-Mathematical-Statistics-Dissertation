...
\label{sec:statistical_learning_supervised_regression}
  ...
  \citep{james2021introduction,hastie2009elements}. In supervised learning, each
...
\begin{figure}[H]
  ...
  \caption{Position of the health-insurance cost prediction task within a standard machine-learning taxonomy.}
...
\citep{goodfellow2016deep,bishop2006pattern}. In the empirical application, the
...
\begin{figure}[H]
    ...
    \caption{Candidate feed-forward neural network architecture for health
...
\begin{figure}[H]
    ...
    \caption{Illustration of the bias-variance trade-off.
    Adapted from \citep{james2021introduction}.}
...
\noindent
Recent actuarial work has demonstrated that neural networks can be incorporated
into actuarial pricing structures without abandoning the underlying
frequency-severity logic. \citet{laporta2025neural} propose neural-network
models for pricing correlated health risks by embedding negative multinomial
...
\citep{chen2016xgboost}, frequently match or outperform deep neural networks on
...
\end{tikzpicture}
\caption{Construction of the input distribution
         $\widehat{F}_{X}$ (equation~\eqref{eq:input_law}).}
...
\citep{glasserman2004monte,rubinstein2016simulation}.
...
they are relatively rare \citep{glasserman2004monte}.
...
provide feature-level attribution for fitted predictions. These components are
not new methods in isolation.
...
\noindent
The research gap is therefore the limited integration of these three components
for health insurance cost prediction. The literature supports each component
individually but gives less attention to the combined question of how a fitted
prediction model behaves across many simulated policyholder profiles, how the
resulting model-implied predicted-cost distribution behaves, and which variables
the fitted model attributes to high-cost simulated profiles.
