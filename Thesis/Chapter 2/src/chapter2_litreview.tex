...
three-stage structure combining \gls{mc} simulation, machine-learning
prediction and Shapley-based attribution in an earned-value
project-control setting: \gls{mc} simulation generates a large synthetic
...
to health insurance cost prediction: \gls{mc} simulation generates
synthetic policyholder profiles from parametric distributions fitted to the
observed training attributes, the fitted \gls{dnn} maps each profile to a
predicted cost, and Shapley values decompose those predictions into
feature-level contributions, with particular attention given to the
high-cost region of the simulated predicted-cost distribution.  The primary
difference is the empirical domain and the choice of prediction model: the
present study uses a \gls{dnn} rather than a general machine-learning
