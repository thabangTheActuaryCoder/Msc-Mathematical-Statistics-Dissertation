...
\citep{hornik1989multilayer, cybenko1989approximation}: for any continuous
function $f: [0,1]^p \to \mathbb{R}$ and any $\varepsilon > 0$, there
exists a single-hidden-layer network $F$ with a non-polynomial activation
function such that
...
What matters is not the particular endpoints but that the inputs cannot
grow without bound; if they could, no fixed-size network would be able to
track the target function everywhere.  Recent studies have examined the
...
\citet{friedman2001greedy}, frequently match or outperform \glspl{dnn}
on
...
the fitted \gls{dnn} generalises within the supplied
...
\gls{mc} simulation.  Instead of computing Shapley values only on the
