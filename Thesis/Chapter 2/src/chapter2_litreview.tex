...
Originally introduced in cooperative game theory by \citep{shapley1953value}
...
\noindent
At the broader actuarial level, \citep{richman2021ai} surveys recent
...
\noindent
A closer comparison is provided by \citep{orji2024explainable}, who apply
...
is \citep{samiuddin2023health}, who fit \gls{ann} and \gls{dnn} models to the
...
estimators are often employed. \citep{castro2009polynomial} proposed a
polynomial-time scheme based on randomly sampled permutations, and
\citep{maleki2013bounding} derived confidence bounds for such estimators.
...
Regression methods are evaluated on a continuous scale using prediction-error measures \citep{james2021introduction,hastie2009elements}. The specific measures employed in this dissertation are defined below.
  ...
  \citep{richman2021ai} notes that neural-network methods can capture
...
insurance frameworks.  \citep{wuthrich2023statistical} provide a comprehensive
treatment of statistical foundations for actuarial learning,
\citep{noll2020case} demonstrate neural-network pricing on French motor
third-party liability data, and \citep{schelldorfer2019nesting} show how
classical actuarial models can be nested within neural-network architectures.
\citep{blierwong2021machine} survey machine-learning applications across
property and casualty insurance pricing and reserving.  In the life insurance
domain, \citep{blomerus2025longterm} demonstrates the viability of deep
...
\citep{laporta2025neural} propose neural-network models for pricing correlated
...
constraints during training.  \citep{richman2024icenet} develop the ICEnet,
...
\citep{orji2024explainable} apply random forests and gradient-boosted trees to
...
\citep{grinsztajn2022why} provide systematic evidence that tree-based models,
...
\citep{friedman2001greedy}, frequently match or outperform deep neural networks
on
...
\citep{bertsimas2008algorithmic} demonstrated the potential of machine-learning
methods to outperform traditional actuarial approaches, while
\citep{duncan2016testing} tested alternative regression frameworks for
...
\noindent
Health expense projection literature provides a further actuarial motivation for studying predicted costs distributionally. \citep{christiansen2018projection}
...
\noindent
The \gls{shap} framework connects Shapley values to machine learning model interpretation \citep{lundberg2017unified}. The additive decomposition in equation~\eqref{eq:shapley_decomp} provides a local explanation of an individual prediction. \citep{lundberg2017unified} proved that this is the only additive feature attribution method satisfying efficiency, dummy and symmetry simultaneously, which is why it constitutes the theoretical foundation for the \gls{shap} computational framework.  \citep{covert2021explaining} provide a unifying removal-based framework that relates SHAP to other explanation methods, \citep{sundararajan2020many} examine the many distinct Shapley value formulations that arise under different baseline and feature-dependence assumptions, and \citep{lundberg2020local} extend Shapley-based explanations from local to global understanding for tree models.
...
within a single, reproducible workflow creates a new analytical
capability applied here to health insurance cost prediction.
...
of these analyses. The survey by \citep{rozemberczki2022shapley} catalogues a
...
profiles.  Recent work by \citep{blomerus2025longterm} demonstrates the
