...
\noindent
As already noted, the statistical notion of bias in this decomposition is
distinct from the trainable bias parameter in a neural-network layer and
from data bias in the benchmark dataset; the bias-variance trade-off is used
here as a general learning principle, not as a justification for the
bias-free \gls{dnn} architecture.
...
\citet{orji2024explainable} apply \glspl{rf} and gradient-boosted trees to
...
\noindent
The \gls{shap} framework connects Shapley values to machine learning model interpretation \citep{lundberg2017unified}. The additive decomposition in equation~\eqref{eq:shapley_decomp} provides a local explanation of an individual prediction. \citet{lundberg2017unified} proved that this is the only additive feature attribution method satisfying efficiency, dummy and symmetry simultaneously, which is why it constitutes the theoretical foundation for the \gls{shap} computational framework.  \citet{covert2021explaining} provide a unifying removal-based framework that relates \gls{shap} to other explanation methods, \citet{sundararajan2020many} examine the many distinct Shapley value formulations that arise under different baseline and feature-dependence assumptions, and \citet{lundberg2020local} extend Shapley-based explanations from local to global understanding for tree models.
