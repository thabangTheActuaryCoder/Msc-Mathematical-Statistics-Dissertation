...
training. Residual-augmented \gls{mc} analyses that extend beyond the
...
the standard deep-learning approaches of \gls{mc} dropout, which
...
guarantee does not directly apply to the implemented architecture. The
justification for the bias-free choice therefore rests on the empirical
diagnostics and model comparisons reported in
Chapters~\ref{chap:methods_and_data} and~\ref{sec:results}, not on the
...
\gls{mc} and Shapley workflow because it provides a differentiable
prediction surface compatible with gradient-based \gls{shap} explainers and
...
Chapter~\ref{chap:methods_and_data}.  A \gls{rf}
...
precisely the condition under which the marginal \gls{shap} estimator used
...
sizes, so any benefit of quasi-\gls{mc} sampling in this setting would rest
