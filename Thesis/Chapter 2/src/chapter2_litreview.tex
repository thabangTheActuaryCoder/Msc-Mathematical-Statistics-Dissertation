...
the universal approximation property: \glspl{dnn} with
...
support the use of a \gls{dnn} as a viable prediction
...
For a \gls{dnn} with \(L\) hidden layers, the parameter vector
...
\glspl{dnn} possess the universal approximation property
...
observed insurance cost. A \gls{dnn} constructs a sequence of
...
The \gls{relu} is widely used in \glspl{dnn} because it is
...
\noindent
The empirical application considers a \gls{dnn} architecture for
...
\begin{figure}[H]
    ...
    \caption{Candidate \gls{dnn} architecture.}
