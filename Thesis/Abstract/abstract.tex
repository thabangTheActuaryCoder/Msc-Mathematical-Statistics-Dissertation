...
This dissertation develops a three-stage statistical framework for predicting health insurance costs.  The analysis uses the Kaggle Insurance Data for Machine Learning benchmark dataset, which contains one million policyholder records generated from pre-specified distributions with a deterministic cost formula.  Because the dataset has a clean, algorithmically constructed structure, the predictive accuracy reported here is substantially higher than would be expected on real-world insurance data, and the specific performance figures should not be generalised beyond this benchmark setting.  The methodology, however, is designed to be transferable to other datasets and domains.
...
\noindent\textbf{Keywords:} deep neural network, health insurance cost prediction, Monte Carlo simulation, Shapley values, explainable machine learning, generalised linear model, random forest, \gls{xgboost}, statistical learning.
